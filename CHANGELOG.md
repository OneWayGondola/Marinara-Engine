...
- Added **Z.AI** as a text provider. Pick it in the connection editor to get the GLM base URL pre-filled, a link to where your key lives, and the GLM model list (now including GLM 5.3 and GLM 5.3 Flash) with **Fetch Models from API** working against your account. A preset set to Maximum reasoning effort now reaches Z.AI's `max` level on GLM 5.2 and 5.3 instead of being lowered to `high` on the way out. Custom connections to `api.z.ai` keep working as before. When a reply comes back with no text, the error now says what the provider reported — the output budget spent on reasoning (with the numbers), a content-policy stop, or the finish reason verbatim — instead of a generic retry line, and **Test Message** gives always-reasoning GLM models 1,024 tokens instead of 200 so the test shows words rather than a blank success.
